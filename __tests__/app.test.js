...
describe('stock-bot routes', () => {
  ...
  it('creates a new user, redirect to main page', async () => {
    const agent = request.agent(app);
    const res = await agent.post('/api/v1/login').send(mockUser).redirects(1);
  ...
  it('creates a new user with url params, redirect to main page', async () => {
    ...
    const res = await agent.post('/api/v1/login/hotdog/hamburger').redirects(1);
  ...
  it('unfollows all stocks for a given user', async () => {
    ...
    console.log(
      '🚀 ~ file: app.test.js ~ line 142 ~ it.only ~ res.body',
      res.body
    );
    let userStocks = await agent.get('/api/v1/login/2');
    console.log('userStocks :>> ', userStocks.text);
    const resp = await agent.delete('/api/v1/login/2');
    
    console.log('resp.body :>> ', resp.body);
    let userStocks2 = await agent.get('/api/v1/login/2');
  ...
  it('unfollows a specific, named stock for a given user', async () => {
    ...
    const res = await agent.delete('/api/v1/login/2/1');
  ...
  it('should return a default row for new user (userid/id issue) ', async () => {
    const agent = request.agent(app);
    //login user
    let res = await agent.post('/api/v1/login').send(mockUser).redirects(1);

    const userEYEDEE = res.body[0].user_id;

    // get sms array
    const sms = await agent.get('/api/v1/sms').send(userEYEDEE);
    ...
    if (checkState === false) {
      res = await agent.post(`/api/v1/sms/newUser/${userId}`);
    }
  ...
  it('should update sms_interval for signed in user, and not for anyone else (userid/id issue)', async () => {
    ...
    //update user array
    let updateSms = await agent.post('/api/v1/sms').send(res.body);
    ...
    res.body.pop();
    res.body.push(updateUser);
    updateSms = await agent.post('/api/v1/sms').send(res.body);
  ...
  it('should logout a user', async () => {
    ...
    const res = await agent1.post('/api/v1/login').send(mockUser).redirects(1);
  ...
  it.skip('should send a text message', async () => {
    ...
    //send sms
    res = await agent.get('/api/v1/sms/send-sms').send(res.body);
  });
